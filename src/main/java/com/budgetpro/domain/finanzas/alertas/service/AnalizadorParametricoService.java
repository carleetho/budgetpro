...
public class AnalizadorParametricoService {
     ...
     * 
     * Si un recurso es de tipo ACTIVO (equipo propio) y tiene costo_horario == 0,
            ...
            if (recurso.getTipo() == TipoRecurso.ACTIVO) {
                ...
                if (costoHorario == null || costoHorario.compareTo(BigDecimal.ZERO) == 0) {
                    AlertaParametrica alerta = AlertaParametrica.crear(
                        ...
                        "' (ACTIVO) tiene costo_horario = 0. Esto puede llevar a pérdidas por no considerar la depreciación del activo.",
                        BigDecimal.ZERO,
