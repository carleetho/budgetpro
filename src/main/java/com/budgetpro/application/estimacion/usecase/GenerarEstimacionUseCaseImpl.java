...
public class GenerarEstimacionUseCaseImpl implements GenerarEstimacionUseCase {
    ...
    public EstimacionResponse generar(GenerarEstimacionCommand command) {
        ...
        Presupuesto presupuesto = presupuestoRepository.findByProyectoId(command.proyectoId())
        ...
        if (command.porcentajeAnticipo() != null && command.porcentajeAnticipo().compareTo(BigDecimal.ZERO) > 0) {
            // TODO: Obtener saldo pendiente de anticipo desde algún servicio o configuración
            // Por ahora, calculamos la amortización teórica
            BigDecimal saldoAnticipoPendiente = new BigDecimal("999999999"); // Placeholder
