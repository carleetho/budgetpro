...
public class CapecoApiAdapter implements CatalogPort {
    ...
    public CapecoApiAdapter(RestTemplate restTemplate,
                            @Value("${catalog.capeco.url}") String apiBaseUrl,
                            @Value("${CAPECO_API_KEY}") String apiKey,
    ...
    public RecursoSnapshot fetchRecurso(String externalId, String catalogSource) {
        ...
        try {
            ...
            ResponseEntity<CapecoRecursoResponse> response = executeWithRetry(url, CapecoRecursoResponse.class, externalId, catalogSource);
            ...
            catalogEventLogger.logApiCall(correlationId, catalogSource, "fetchRecurso", externalId, durationMs, true, null);
            ...
            catalogEventLogger.logApiCall(correlationId, catalogSource, "fetchRecurso", externalId, durationMs, false, e);
            throw e;
        }
    }

    /**
     * Fallback: Intenta obtener desde cache cuando el circuito está abierto o hay error.
     */
    private RecursoSnapshot fetchRecursoFromCache(String externalId, String catalogSource, Exception e) {
        String correlationId = catalogEventLogger.generateCorrelationId();
        log.warn("Circuit breaker open or error, falling back to cache for recurso {} from {}", externalId, catalogSource, e);
        String cacheKey = String.format("%s:%s", catalogSource, externalId);
        ...
        return catalogCache.getRecursoL2(cacheKey)
                .orElseThrow(() -> new CatalogServiceException(
                        catalogSource,
                        String.format("Catalog unavailable and no cache for recurso %s", externalId),
                        e
                ));
    }
    ...
    public List<RecursoSnapshot> searchRecursos(RecursoSearchCriteria criteria, String catalogSource) {
        ...
        try {
            ...
            ResponseEntity<CapecoRecursoResponse[]> response = executeWithRetry(url, CapecoRecursoResponse[].class, "SEARCH", catalogSource);
            ...
            List<RecursoSnapshot> resultados = List.of(body).stream()
                    .map(item -> mapToRecursoSnapshot(item, catalogSource))
                    .collect(Collectors.toList());

            success = true;
            long durationMs = System.currentTimeMillis() - startTime;
            catalogMetrics.recordApiCall(catalogSource, "searchRecursos", durationMs, true);
            catalogEventLogger.logApiCall(correlationId, catalogSource, "searchRecursos", "SEARCH", durationMs, true, null);
            ...
            catalogEventLogger.logApiCall(correlationId, catalogSource, "searchRecursos", "SEARCH", durationMs, false, e);
            throw e;
        }
    }

    /**
     * Fallback: Retorna lista vacía cuando el circuito está abierto (búsquedas no se cachean).
     */
    private List<RecursoSnapshot> searchRecursosFromCache(RecursoSearchCriteria criteria, String catalogSource, Exception e) {
    ...
    public APUSnapshot fetchAPU(String externalApuId, String catalogSource) {
        ...
        try {
            ...
            ResponseEntity<CapecoApuResponse> response = executeWithRetry(url, CapecoApuResponse.class, externalApuId, catalogSource);
            ...
            APUSnapshot snapshot = APUSnapshot.crear(
                    ...
                    externalApuId,
                    catalogSource,
                    body.getRendimiento() != null ? body.getRendimiento() : BigDecimal.ONE,
                    body.getUnidad(),
                    LocalDateTime.now()
            );
            ...
            catalogEventLogger.logApiCall(correlationId, catalogSource, "fetchAPU", externalApuId, durationMs, true, null);
            ...
            catalogEventLogger.logApiCall(correlationId, catalogSource, "fetchAPU", externalApuId, durationMs, false, e);
            throw e;
        }
    }
    ...
    private APUSnapshot fetchAPUFromCache(String externalApuId, String catalogSource, Exception e) {
        String correlationId = catalogEventLogger.generateCorrelationId();
        log.warn("Circuit breaker open or error, falling back to cache for APU {} from {}", externalApuId, catalogSource, e);
        String cacheKey = String.format("%s:%s", catalogSource, externalApuId);
        ...
        return catalogCache.getApuL2(cacheKey)
                .orElseThrow(() -> new CatalogServiceException(
                        catalogSource,
                        String.format("Catalog unavailable and no cache for APU %s", externalApuId),
                        e
                ));
    }
    ...
    public boolean isRecursoActive(String externalId, String catalogSource) {
        ...
        try {
            ...
            catalogEventLogger.logApiCall(correlationId, catalogSource, "isRecursoActive", externalId, durationMs, true, null);
            
            return activo;
        } catch (CatalogNotFoundException e) {
            long durationMs = System.currentTimeMillis() - startTime;
            catalogMetrics.recordApiCall(catalogSource, "isRecursoActive", durationMs, true); // NotFound no es error funcional
            catalogEventLogger.logApiCall(correlationId, catalogSource, "isRecursoActive", externalId, durationMs, true, null);
            return false;
        } catch (Exception e) {
            error = e;
            ...
            catalogEventLogger.logApiCall(correlationId, catalogSource, "isRecursoActive", externalId, durationMs, false, e);
            throw e;
        }
    }
    ...
    private <T> ResponseEntity<T> executeWithRetry(String url,
        ...
        while (true) {
            try {
                HttpEntity<Void> request = new HttpEntity<>(java.util.Objects.requireNonNull(buildHeaders(), "Headers no pueden ser nulos"));
                return restTemplate.exchange(
                        java.util.Objects.requireNonNull(url, "URL no puede ser nula"),
                        java.util.Objects.requireNonNull(HttpMethod.GET, "HttpMethod no puede ser nulo"),
                        request,
