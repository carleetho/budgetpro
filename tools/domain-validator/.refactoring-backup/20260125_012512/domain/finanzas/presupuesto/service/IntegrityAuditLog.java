 ...
 * 
 * - Generación de hashes al aprobar presupuestos
 * - Validación de hashes durante operaciones críticas
 * - Violaciones de integridad detectadas
 * 
 * **Patrón Fire-and-Forget:**
 * Los métodos de logging son asíncronos y no bloquean el flujo principal.
 * Si el logging falla, no debe afectar la operación principal.
 * 
 * **Swiss-Grade Engineering:**
 * Este servicio crea un registro inmutable y completo de todos los eventos
 * de integridad para análisis forense y cumplimiento normativo.
 * 
 * **Integración con Seguridad:**
 * Las violaciones de integridad deben disparar alertas al equipo de seguridad.
 * Esta integración se implementará en una tarea separada.
 * 
...
public class IntegrityAuditLog {
     ...
     * 
     * Este evento se registra cuando un presupuesto es aprobado y se generan
     * los hashes criptográficos de aprobación y ejecución.
     * 
    ...
    public void logHashGeneration(Presupuesto presupuesto) {
        ...
        IntegrityAuditEntry entry = IntegrityAuditEntry.crear(
            UUID.randomUUID(),
            presupuesto.getId().getValue(),
            "HASH_GENERATED",
            presupuesto.getIntegrityHashApproval(),
            presupuesto.getIntegrityHashExecution(),
            presupuesto.getIntegrityHashGeneratedBy(),
            presupuesto.getIntegrityHashGeneratedAt() != null 
                ? presupuesto.getIntegrityHashGeneratedAt() 
                : LocalDateTime.now(),
            "SUCCESS",
            null,
     ...
     * antes de permitir una operación crítica (ej: aprobar compra, realizar egreso).
     * 
     ...
     * 
     * CRÍTICO: Este método registra violaciones de integridad criptográfica.
     * Las violaciones indican posibles intentos de tampering o modificaciones
     * no autorizadas a presupuestos sellados.
     * 
     * Este método debe:
     * 1. Registrar la violación en el audit log
     * 2. Disparar una alerta al equipo de seguridad (implementación futura)
     * 
    ...
    public void logIntegrityViolation(BudgetIntegrityViolationException exception, UUID detectedBy) {
        ...
        IntegrityAuditEntry entry = IntegrityAuditEntry.crear(
            UUID.randomUUID(),
            exception.getPresupuestoId().getValue(),
            "HASH_VIOLATION",
            exception.getExpectedHash(),
            exception.getActualHash(),
            detectedBy,
            LocalDateTime.now(),
            "FAILURE",
