...
/**
 * Servicio de dominio para cálculo dinámico de costos de APU usando fórmulas de ingeniería civil.
 * 
 * Implementa las fórmulas según el tipo de recurso:
 ...
 * 
 * Respetando el orden de dependencias: MATERIAL, MANO_OBRA, EQUIPO_MAQUINA → EQUIPO_HERRAMIENTA
 */
@Service
public class CalculoApuDinamicoService {
     ...
     * @param rendimientoVigente El rendimiento vigente del APU (editable por el ingeniero)
     * @param costoTotalMO El costo total de mano de obra del APU (necesario para herramientas)
     ...
     */
    public BigDecimal calcularCostoInsumo(APUInsumoSnapshot insumo,
        ...
        switch (tipoRecurso) {
            ...
            case SUBCONTRATO:
                ...
                BigDecimal precio = insumo.getPrecioPuestoEnObra() != null 
                    ? insumo.getPrecioPuestoEnObra() 
                    : insumo.getPrecioUnitario();
                costo = normalizarPrecioAMonedaProyecto(precio, insumo.getMoneda(), 
    ...
    /**
     * Calcula el costo de MATERIAL usando la fórmula:
     * Precio Puesto en Obra × Aporte Unitario × (1 + Desperdicio) × TipoCambio
     */
    ...
    private BigDecimal calcularCostoManoObra(APUInsumoSnapshot insumo,
        ...
        if (costoDiaCuadrilla == null) {
            ...
            if (costoDiaCuadrilla == null) {
                throw new IllegalArgumentException("No se puede calcular costo de mano de obra sin composición de cuadrilla o costo día cuadrilla calculado");
            }
        }
    ...
    /**
     * Calcula el costo día cuadrilla como suma de: Σ(cantidad × costoDia × tipoCambio)
     */
    public BigDecimal calcularCostoDiaCuadrilla(List<ComposicionCuadrillaSnapshot> composicionCuadrilla,
        ...
        return composicionCuadrilla.stream()
            .map(comp -> {
    ...
    /**
     * Calcula el costo de EQUIPO_MAQUINA usando la fórmula:
     * Costo Hora Máquina × (Horas Uso / Rendimiento Vigente)
     */
    ...
    /**
     * Calcula el costo de EQUIPO_HERRAMIENTA usando la fórmula:
     * Costo Total Mano de Obra × Porcentaje
     */
    private BigDecimal calcularCostoEquipoHerramienta(APUInsumoSnapshot insumo,
                                                      BigDecimal costoTotalMO) {
        if (costoTotalMO == null) {
            throw new IllegalArgumentException("No se puede calcular costo de herramienta sin costo total de mano de obra");
        }
        ...
        return costoTotalMO
            .multiply(porcentajeManoObra)
            .setScale(PRECISION_INTERMEDIA, ROUNDING_MODE);
    }

    /**
     * Normaliza un precio de una moneda origen a la moneda del proyecto usando tipo de cambio.
     */
    public BigDecimal normalizarPrecioAMonedaProyecto(BigDecimal precio,
                                                       String monedaOrigen,
        ...
        if (tipoCambio == null || tipoCambio.compareTo(BigDecimal.ZERO) <= 0) {
            throw new IllegalArgumentException("Tipo de cambio inválido para convertir de " + monedaOrigen + " a " + monedaProyecto);
        }
    ...
    /**
     * Calcula el costo total del APU respetando el orden de dependencias:
    ...
    public BigDecimal calcularCostoTotalAPU(APUSnapshot apuSnapshot, String monedaProyecto) {
        ...
        for (APUInsumoSnapshot insumo : insumos) {
            ...
            switch (tipoRecurso) {
                case MATERIAL:
                    costoTotalMaterial = costoTotalMaterial.add(
                        calcularCostoInsumo(insumo, rendimientoVigente, BigDecimal.ZERO, monedaProyecto)
                    );
                    break;
                case MANO_OBRA: {
                ...
                case EQUIPO_MAQUINA: {
                    BigDecimal costoEquipo = calcularCostoInsumo(insumo, rendimientoVigente, BigDecimal.ZERO, monedaProyecto);
        ...
        for (APUInsumoSnapshot insumo : insumos) {
            ...
            if (tipoRecurso == TipoRecurso.EQUIPO_HERRAMIENTA) {
                BigDecimal costoHerramienta = calcularCostoInsumo(insumo, rendimientoVigente, costoTotalMO, monedaProyecto);
        ...
        return costoTotalMaterial
            .add(costoTotalMO)
            .add(costoTotalEquipoMaquina)
            .add(costoTotalHerramienta)
