...
class SecurityValidator(BaseValidator):
    ...
    def _check_compilation(self, files: List[str]) -> List[Violation]:
        ...
        if not os.path.exists(mvnw_path):
            # If mvnw is not in 'backend', check root (different repo structure)
            if os.path.exists("mvnw"):
                mvnw_path = "./mvnw"
                backend_dir = "."
            else:
        ...
        try:
            # We use -DskipTests to keep it "fast" (< 2m as per requirement)
            # and -q for quiet output unless there is an error
            result = subprocess.run(
                [mvnw_path, "compile", "-DskipTests"],
                cwd=backend_dir,
                capture_output=True,
    ...
    def _check_credentials(self, files: List[str]) -> List[Violation]:
        ...
        # Files to ignore (binaries, non-text files that might cause false positives or read errors)
        ignore_extensions = ['.png', '.jpg', '.jpeg', '.gif', '.ico', '.pdf', '.zip', '.jar', '.war']
                    ...
                    for line_num, line in enumerate(f, 1):
                        for pattern_name, pattern in self.SENSITIVE_PATTERNS.items():
                            if re.search(pattern, line):
                                ...
                                # Only report one pattern per line to avoid redundancy
                                break
            except Exception as e:
    ...
    def _check_gitignore(self) -> List[Violation]:
        ...
        try:
            with open(gitignore_path, "r") as f:
                content = f.read()
                for pattern in critical_patterns:
                    # Simple check for pattern existence in the file
                    # We look for the pattern as a whole line or part of a path
                    if pattern not in content:
                        violations.append(Violation(
                            file_path=gitignore_path,
                            message=f"MISSING EXCLUSION: .gitignore does not contain '{pattern}'.",
