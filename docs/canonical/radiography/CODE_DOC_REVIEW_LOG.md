...
| H-08 | `DATA_MODEL`: almacén/marketing/reajuste “sin DDL” | Almacén `V20.1`, marketing `V30`, reajuste `V32` referenciados |
...
| H-14 | Flyway **`V17__` duplicado** (O-04): arranque bloqueado por colisión de versión | Renombrado `V17__create_presupuesto_integrity_audit.sql` → **`V17.1__create_presupuesto_integrity_audit.sql`**; permanece `V17__add_evm_time_series.sql`. Orden: **17** → **17.1**. Bases ya migradas con el nombre antiguo pueden requerir ajuste manual en `flyway_schema_history` / repair. |
