...
public class ConfigurarSobrecostoUseCaseImpl implements ConfigurarSobrecostoUseCase {
        ...
        public AnalisisSobrecostoResponse configurar(ConfigurarSobrecostoCommand command) {
                ...
                analisis = analisis.actualizarIndirectos(
                                command.porcentajeIndirectosOficinaCentral() != null
                                                ? command.porcentajeIndirectosOficinaCentral()
                                                : BigDecimal.ZERO,
                                command.porcentajeIndirectosOficinaCampo() != null
                                                ? command.porcentajeIndirectosOficinaCampo()
                                                : BigDecimal.ZERO);

                analisis = analisis.actualizarFinanciamiento(
                                command.porcentajeFinanciamiento() != null ? command.porcentajeFinanciamiento()
                                                : BigDecimal.ZERO,
                                command.financiamientoCalculado() != null ? command.financiamientoCalculado() : false);

                analisis = analisis.actualizarUtilidad(
                                command.porcentajeUtilidad() != null ? command.porcentajeUtilidad() : BigDecimal.ZERO);

                analisis = analisis.actualizarCargosAdicionales(
                                command.porcentajeFianzas() != null ? command.porcentajeFianzas() : BigDecimal.ZERO,
                                command.porcentajeImpuestosReflejables() != null
                                                ? command.porcentajeImpuestosReflejables()
                                                : BigDecimal.ZERO);
                ...
                return new AnalisisSobrecostoResponse(analisis.getId().getValue(), analisis.getPresupuestoId(),
                                analisis.getPorcentajeIndirectosOficinaCentral(),
                                analisis.getPorcentajeIndirectosOficinaCampo(), analisis.getPorcentajeIndirectosTotal(),
                                analisis.getPorcentajeFinanciamiento(), analisis.getFinanciamientoCalculado(),
                                analisis.getPorcentajeUtilidad(), analisis.getPorcentajeFianzas(),
                                analisis.getPorcentajeImpuestosReflejables(),
                                analisis.getPorcentajeCargosAdicionalesTotal(), analisis.getVersion().intValue());
        }
}
