 ...
 * 
 * Realiza recálculo automático en cascada de costos afectados y actualiza el
 * hash de ejecución si el presupuesto está aprobado.
 */
@Service
public class ActualizarRendimientoUseCaseImpl implements ActualizarRendimientoUseCase {
        ...
        public ActualizarRendimientoUseCaseImpl(ApuSnapshotRepository apuSnapshotRepository,
                        CalculoApuDinamicoService calculoApuDinamicoService, PartidaRepository partidaRepository,
        ...
        public void actualizarRendimiento(UUID apuSnapshotId, BigDecimal nuevoRendimiento, UUID usuarioId) {
                ...
                apuSnapshot = apuSnapshot.actualizarRendimiento(nuevoRendimiento, usuarioId);

                // 5. Obtener partida y presupuesto para validar integridad si es necesario
                Partida partida = partidaRepository.findById(apuSnapshot.getPartidaId()).orElseThrow(
                                () -> new IllegalStateException("Partida no encontrada para APU: " + apuSnapshotId));
                ...
                // 7. Si el presupuesto está aprobado, validar integridad y actualizar hash de
                // ejecución
                if (presupuesto.isAprobado()) {
                        // Validar que no se haya modificado la estructura (solo se permite actualizar
                        // hash de ejecución)
                ...
                log.debug("Costo total recalculado para APU {}: {} (moneda: {})", apuSnapshotId, nuevoCostoTotal,
                                monedaProyecto);
        }
}
