 ...
 * - Validar stock disponible antes de despachar
 * - Registrar despachos parciales o totales
 * - Actualizar estado de la requisición automáticamente
 * - Transicionar a PENDIENTE_COMPRA cuando stock insuficiente
 * 
...
public final class DespachoRequisicionService {
     ...
     * 
     * <p>Flujo:
     * 1. Valida que la requisición esté en estado válido (APROBADA, DESPACHADA_PARCIAL, PENDIENTE_COMPRA)
     * 2. Para cada DespachoItem:
     *    - Busca el RequisicionItem en la requisición
     *    - Busca InventarioItem por (proyectoId, recursoExternalId, unidadMedida, bodegaId)
     *    - Valida stock disponible
     *    - Si insuficiente: transiciona requisición a PENDIENTE_COMPRA y lanza excepción
     *    - Si suficiente: egresa del inventario y actualiza cantidadDespachada
     * 3. Actualiza estado de la requisición según progreso (DESPACHADA_PARCIAL o DESPACHADA_TOTAL)
     * 4. Persiste cambios
     * 
     * @param requisicionId ID de la requisición a despachar
     * @param items Lista de ítems a despachar con sus cantidades
     * @throws RequisicionNoAprobadaException si la requisición no está en estado válido
     * @throws CantidadInsuficienteException si no hay stock suficiente (transiciona a PENDIENTE_COMPRA)
     * @throws IllegalArgumentException si algún ítem no existe o cantidad inválida
     */
    public void despacharRequisicion(RequisicionId requisicionId, List<DespachoItem> items) {
        ...
        BodegaId bodegaId = defaultBodegaPort.getDefaultForProject(requisicion.getProyectoId())
                .orElseThrow(() -> new IllegalArgumentException(
                        "No hay bodega por defecto para el proyecto " + requisicion.getProyectoId()));
        ...
        for (DespachoItem despachoItem : items) {
            RequisicionItem requisicionItem = requisicion.getItems().stream()
                    ...
                    .findFirst()
                    .orElseThrow(() -> new IllegalArgumentException(
                            "Ítem no encontrado en la requisición: " + despachoItem.getRequisicionItemId()));
            ...
            if (despachoItem.getCantidadADespachar().compareTo(cantidadPendiente) > 0) {
                throw new IllegalArgumentException(
                        String.format("No se puede despachar %s. Cantidad pendiente: %s de %s solicitados",
            ...
            InventarioItem inventarioItem = inventarioRepository
                    .findByProyectoIdAndRecursoExternalIdAndUnidadBaseAndBodegaId(
                            ...
                            requisicionItem.getUnidadMedida(),
                            bodegaId)
                    .orElseThrow(() -> new IllegalArgumentException(
                            String.format("Inventario no encontrado para recurso %s, unidad %s, bodega %s",
                                        requisicionItem.getRecursoExternalId(),
                                        requisicionItem.getUnidadMedida(),
                                        bodegaId)));
            ...
            if (inventarioItem.getCantidadFisica().compareTo(despachoItem.getCantidadADespachar()) < 0) {
                // Stock insuficiente: transicionar a PENDIENTE_COMPRA y crear RequerimientoCompra
                if (requisicion.getEstado() == EstadoRequisicion.APROBADA ||
                    ...
                    backlogService.crearRequerimiento(
                            requisicion.getProyectoId(),
                            requisicion.getId(),
                            requisicionItem.getRecursoExternalId(),
                            cantidadNecesaria,
                ...
                throw new CantidadInsuficienteException(
                        String.format("Stock insuficiente para despachar %s de %s. " +
            ...
            String referencia = String.format("Despacho requisición #%s - %s",
                    requisicionId.getValue(), requisicion.getSolicitante());
            inventarioItem.egresarPorRequisicion(
                    despachoItem.getCantidadADespachar(),
                    requisicionId.getValue(),
                    requisicionItem.getId().getValue(),
                    requisicionItem.getPartidaId(),
                    referencia
            );
