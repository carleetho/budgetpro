 ...
 * 
 * 1. **Hash de Aprobación (integrityHashApproval):**
 *    - Se genera una sola vez al aprobar el presupuesto
 *    - Es INMUTABLE después de la aprobación
 *    - Captura la estructura completa del presupuesto en el momento de aprobación
 *    - Se usa para detectar modificaciones no autorizadas a la estructura
 * 
 * 2. **Hash de Ejecución (integrityHashExecution):**
 *    - Se genera al aprobar y se actualiza después de cada transacción financiera
 *    - Es DINÁMICO y refleja el estado actual de ejecución
 *    - Captura el estado financiero (consumos, saldos, movimientos)
 *    - Se usa para detectar inconsistencias en la ejecución
 * 
 * 3. **Hard-Freeze Pattern:**
 *    - Una vez que integrityHashApproval está establecido, el presupuesto está "sellado"
 *    - Cualquier intento de modificar campos estructurales lanza BudgetIntegrityViolationException
 *    - Solo se permite actualizar el hash de ejecución después de transacciones financieras
 * 
...
public final class Presupuesto {
    ...
    /**
     * Hash criptográfico inmutable de la estructura del presupuesto al momento de aprobación.
     * Se genera una sola vez y nunca cambia después de la aprobación.
     * Formato: 64 caracteres hexadecimales (SHA-256).
     */
    private String integrityHashApproval;      // Immutable approval structure hash

    /**
     * Hash criptográfico dinámico del estado de ejecución del presupuesto.
     * Se actualiza después de cada transacción financiera para reflejar el estado actual.
     * Formato: 64 caracteres hexadecimales (SHA-256).
     */
    private String integrityHashExecution;     // Dynamic execution state hash
    ...
    /**
     * Versión del algoritmo de hash usado. Actualmente "SHA-256-v1".
     * Permite migración futura a algoritmos más seguros sin romper compatibilidad.
     */
    private String integrityHashAlgorithm;     // "SHA-256-v1"
    ...
    private Presupuesto(PresupuestoId id, UUID proyectoId, String nombre, 
                        ...
                        String integrityHashApproval, String integrityHashExecution,
                        LocalDateTime integrityHashGeneratedAt, UUID integrityHashGeneratedBy,
                        String integrityHashAlgorithm) {
     ...
     * Los campos de integridad hash se inicializan como null hasta la aprobación.
     */
    ...
    /**
     * Factory method para reconstruir un Presupuesto desde persistencia (firma simplificada).
     * Los campos de integridad hash se establecen como null (para compatibilidad con código existente).
     * 
     * @deprecated Use la firma completa con campos de integridad hash cuando estén disponibles en la persistencia.
     */
    @Deprecated
     ...
     * Incluye todos los campos de integridad hash si el presupuesto fue aprobado.
     */
    public static Presupuesto reconstruir(PresupuestoId id, UUID proyectoId, String nombre,
                                          EstadoPresupuesto estado, Boolean esContractual, Long version,
                                          String integrityHashApproval, String integrityHashExecution,
                                          ...
                                          String integrityHashAlgorithm) {
        return new Presupuesto(id, proyectoId, nombre, estado, esContractual, version,
                integrityHashApproval, integrityHashExecution, integrityHashGeneratedAt,
                integrityHashGeneratedBy, integrityHashAlgorithm);
    }
     ...
     * 
     * **Hard-Freeze Guard:** Si el presupuesto tiene un hash de aprobación establecido,
     * cualquier modificación lanza BudgetIntegrityViolationException para proteger la integridad.
     * 
     * @param nuevoNombre El nuevo nombre del presupuesto
     * @throws IllegalArgumentException si el nombre es nulo o vacío
     * @throws BudgetIntegrityViolationException si el presupuesto está sellado (tiene hash de aprobación)
     */
    public void actualizarNombre(String nuevoNombre) {
        // Hard-freeze check: no se puede modificar un presupuesto sellado
        if (integrityHashApproval != null) {
            throw new BudgetIntegrityViolationException(
                    this.id,
                    integrityHashApproval,
                    "N/A",
     ...
     * 
     * Este método implementa el patrón de "hard-freeze" mediante sellado criptográfico:
     * 
     * 1. Cambia el estado a CONGELADO y marca como contractual
     * 2. Genera el hash de aprobación (immutable) que captura la estructura del presupuesto
     * 3. Genera el hash de ejecución (dinámico) que refleja el estado financiero inicial
     * 4. Registra metadata de cuándo y quién aprobó el presupuesto
     * 
     * **Una vez aprobado, el presupuesto está "sellado" y no puede modificarse estructuralmente.**
     * 
     ...
     * 
     * **Hard-Freeze Guard:** Si el presupuesto tiene un hash de aprobación establecido,
     * cualquier modificación lanza BudgetIntegrityViolationException.
     */
    ...
    /**
     * Valida la integridad criptográfica del presupuesto comparando el hash de aprobación actual
     * con el hash almacenado.
     * 
     * Este método recalcula el hash de aprobación usando la estructura actual del presupuesto
     * y lo compara con el hash inmutable almacenado. Si difieren, significa que la estructura
     * del presupuesto ha sido modificada después de la aprobación.
     * 
     * **Uso típico:**
     * - Antes de operaciones críticas (pagos, reportes financieros)
     * - En procesos de auditoría
     * - En validaciones de integridad periódicas
     * 
     * @param hashService Servicio para calcular el hash de aprobación
     * @throws BudgetIntegrityViolationException si el hash calculado no coincide con el almacenado
     */
     ...
     * 
     * El hash de ejecución es dinámico y refleja el estado financiero actual del presupuesto
     * (consumos, saldos disponibles, movimientos de caja relacionados). Se debe actualizar
     * después de cada operación financiera que afecte el presupuesto:
     * 
     * - Registro de compras
     * - Aprobación de pagos
     ...
     * 
     * **Precondición:** El presupuesto debe estar aprobado (tener hash de aprobación).
     * 
     ...
     * 
     * @return true si el presupuesto está sellado criptográficamente, false en caso contrario
     */
    ...
    public String toString() {
        return String.format("Presupuesto{id=%s, proyectoId=%s, nombre='%s', estado=%s, esContractual=%s, version=%d, sealed=%s}", 
