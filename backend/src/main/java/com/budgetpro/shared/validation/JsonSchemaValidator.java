 ...
 * 
 * Valida que los JSON generados cumplan con los esquemas definidos
 * antes de persistirlos en la base de datos.
 * 
 * **Esquemas soportados:**
 * - fechas-snapshot-schema.json: Estructura de fechas del cronograma
 ...
 * **Uso:**
 * ```java
 * validator.validateFechasSnapshot(jsonString);
 * ```
 */
@Component
public class JsonSchemaValidator {
    ...
    private void validate(String jsonString, Schema schema, String fieldName) {
        Objects.requireNonNull(jsonString, "El JSON no puede ser nulo");
        Objects.requireNonNull(schema, "El esquema no puede ser nulo");
        
        if (jsonString.isBlank()) {
            throw new IllegalArgumentException(
                String.format("El JSON de %s no puede estar vacío", fieldName));
        }
