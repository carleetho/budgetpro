...
public interface EstimacionJpaRepository extends JpaRepository<EstimacionEntity, UUID> {
    ...
    List<EstimacionEntity> findByPresupuestoId(UUID presupuestoId); // Implicitly filtered by project via service layer
    ...
    boolean existsPeriodoSolapadoExcludingId(@Param("proyectoId") UUID proyectoId, @Param("inicio") LocalDate inicio,
            @Param("fin") LocalDate fin, @Param("excludeId") UUID excludeId);
}
