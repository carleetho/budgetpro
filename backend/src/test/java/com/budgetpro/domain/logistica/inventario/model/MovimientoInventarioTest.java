...
class MovimientoInventarioTest {
    ...
    void crearSalidaConRequisicion_conDatosValidos_debeCrearMovimientoConReferencias() {
        ...
        MovimientoInventario movimiento = MovimientoInventario.crearSalidaConRequisicion(
                ...
                requisicionItemId,
                partidaId,
                "Despacho según requisición REQ-001"
        );
    ...
    void crearAjuste_sinJustificacion_debeLanzarExcepcion() {
        ...
        assertThrows(IllegalArgumentException.class, () ->
                MovimientoInventario.crearAjuste(
                        movimientoId,
                        inventarioItemId,
                        new BigDecimal("10"),
    ...
    void esEntrada_paraTodosLosTiposEntrada_debeRetornarTrue() {
        ...
        MovimientoInventario entradaCompra = MovimientoInventario.crearEntradaPorCompra(
                movimientoId, inventarioItemId, new BigDecimal("10"), new BigDecimal("5.00"),
                UUID.randomUUID(), "Compra"
        );
        assertTrue(entradaCompra.esEntrada());

        MovimientoInventario entradaTransferencia = MovimientoInventario.crearEntradaTransferencia(
                movimientoId, inventarioItemId, new BigDecimal("10"), new BigDecimal("5.00"),
                UUID.randomUUID(), "Transferencia"
        );
        assertTrue(entradaTransferencia.esEntrada());

        MovimientoInventario entradaPrestamo = MovimientoInventario.crearEntradaPrestamo(
                movimientoId, inventarioItemId, new BigDecimal("10"), new BigDecimal("5.00"),
                "Préstamo"
        );
        assertTrue(entradaPrestamo.esEntrada());

        MovimientoInventario ajuste = MovimientoInventario.crearAjuste(
                movimientoId, inventarioItemId, new BigDecimal("10"), new BigDecimal("5.00"),
                "Esta es una justificación detallada que tiene más de 20 caracteres",
                "Ajuste"
        );
    ...
    void esSalida_paraTodosLosTiposSalida_debeRetornarTrue() {
        ...
        MovimientoInventario salidaConsumo = MovimientoInventario.crearSalidaPorConsumo(
                movimientoId, inventarioItemId, new BigDecimal("10"), new BigDecimal("5.00"),
                "Consumo"
        );
        assertTrue(salidaConsumo.esSalida());

        MovimientoInventario salidaTransferencia = MovimientoInventario.crearSalidaTransferencia(
                movimientoId, inventarioItemId, new BigDecimal("10"), new BigDecimal("5.00"),
                UUID.randomUUID(), "Transferencia"
        );
        assertTrue(salidaTransferencia.esSalida());

        MovimientoInventario salidaPrestamo = MovimientoInventario.crearSalidaPrestamo(
                movimientoId, inventarioItemId, new BigDecimal("10"), new BigDecimal("5.00"),
                "Préstamo"
        );
